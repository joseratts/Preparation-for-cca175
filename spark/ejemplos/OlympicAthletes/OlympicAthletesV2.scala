...
//Ejercicio 3
val result3 = sqlContext.sql("SELECT COUNTRY, DISCIPLINE, COUNT(MEDALS) AS TOTAL_MEDALS FROM OlympicAthletes WHERE DISCIPLINE = 'Swimming' GROUP BY COUNTRY ORDER BY TOTAL_MEDALS DESC LIMIT 20");
