...
val moviesDF = moviesFileNoHeader.map{ line => val values = line.split(",")
				Movie(values(0), values(1), values(2))}.toDF()

val movieRatingDF = ratingDF.join(moviesDF, "movieId")

//Ejercicio 1
movieRatingDF.select("movieId", "title", "rating").groupBy("movieId", "title").agg((avg("rating")).as("AVG-RATING"), (count("rating")).as("REVIEWS")).filter($"REVIEWS" > 10).orderBy($"AVG-RATING".desc).show()

//Ejercicio 2
movieRatingDF.select("movieId", "title", "rating", "genres").where($"genres".contains("Comedy")).groupBy("movieId", "title").agg((avg("rating")).as("AVG-RATING"), (count("rating")).as("REVIEWS")).filter($"REVIEWS" > 10).orderBy($"AVG-RATING".desc).show()

//Ejercicio 3
movieRatingDF.select("movieId", "title", "rating", "genres").where($"genres".contains("Drama")).groupBy("movieId", "title").agg((avg("rating")).as("AVG-RATING"), (count("rating")).as("REVIEWS")).filter($"REVIEWS" > 10).orderBy($"AVG-RATING".desc).show()

//Ejercicio 4
ratingDF.select("userId", "rating").groupBy("userId").agg((avg("rating")).as("AVG-RATING"), (count("rating")).as("REVIEWS")).orderBy($"REVIEWS".desc, $"AVG-RATING".desc).limit(20).show()